...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const navLinks = document.querySelectorAll('[href^="#"]');//hrefの前方一致
...
navLinks.forEach(link => {
  link.addEventListener('click', (e) => {
    ...
    //フォントサイズを取得
    const defoultFontSize = parseInt(window.getComputedStyle(document.body).fontSize, 10);
    // navのheightをとる
    const offSet = document.querySelector('.navbar').offSetHeight + defoultFontSize;
    // window.scrollでスクロールさせる
    window.scroll({
      top: pageOffset + position - offSet,
      behavior: 'smooth'
    });
  },false);
});
